...
module Data_structure
 ...
 #verbose
  ...
  #  check_block_data = Proc.new{{eval method(__method__).parameters.fetch(0).fetch(1)}.kind_of?(Block_data)}
 ...
 def regex_data_table(block,ldr)
  ...
  def block_refactoring(block)
    ...
    ["X","Y"].each{|i|
      ["UNITS",          "FACTOR"].each{|ldr| 
        block[ldr] << block[ldr.prepend(i)].to_s}
      ["FIRST","LAST","MIN","MAX","DELTA"].each{|ldr| 
        block[ldr] << block[ldr.concat(i)].to_s}
                 }
    ...
    #wait = gets if @v_refactoring == "wait"
